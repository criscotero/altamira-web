...
export default async function ServicesPage({ params }: { params: { locale: string } }) {
  ...
  const packages = t.raw("packages") as any;
  const deliverables = t.raw("deliverables") as any;
  const engagement = t.raw("engagement") as any;
  ...
  return (
    <>
      <section className="bg-white">
        <Container>
          <div className="py-12">
            ...
            {renderCta(intro.cta, BOOK_CALL_URL)}
          </div>
        </Container>
      </section>
      ...
      <section className="bg-white">
        <Container>
          <div className="py-12">
            ...
            {renderCta(pillars.cta, "/how-we-work")}
          </div>
        </Container>
      </section>
      ...
      <section className="bg-white">
        <Container>
          <div className="py-12">
            <h2 className="text-2xl font-semibold text-brand-navy">{packages.headline}</h2>
            <p className="mt-2 text-sm text-zinc-600">{packages.subheadline}</p>
            <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {packages.cards.map((card: any) => (
                <div key={card.title} className="rounded-xl2 border border-zinc-200 bg-zinc-50 p-6">
                  <h3 className="text-sm font-semibold text-brand-navy">{card.title}</h3>
                  <p className="mt-2 text-sm text-zinc-600">{card.description}</p>
                  {card.bullets ? (
            ...
            {renderCta(packages.cta, "/contact")}
          </div>
        </Container>
      </section>
      ...
      <section className="bg-white">
        <Container>
          <div className="py-12">
            <h2 className="text-2xl font-semibold text-brand-navy">{engagement.headline}</h2>
            <p className="mt-2 text-sm text-zinc-600">{engagement.subheadline}</p>
            <div className="mt-8 grid gap-6 md:grid-cols-3">
              {engagement.cards.map((card: any) => (
                <div key={card.title} className="rounded-xl2 border border-zinc-200 bg-zinc-50 p-6">
                  <h3 className="text-sm font-semibold text-brand-navy">{card.title}</h3>
                  <p className="mt-2 text-sm text-zinc-600">{card.description}</p>
                  {card.bullets ? (
            ...
            {renderCta(engagement.cta, "/contact")}
          </div>
        </Container>
      </section>
